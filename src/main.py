...
async def crawl_products():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        while True:
            ...
            if no_results_found:
                print("No more product found. Ending crawl.")
    ...
    if all_products:
        save_products_to_csv(all_products, "complete_venues.csv")
        print(f"Saved {len(all_products)} venues to 'complete_venues.csv'.")
    else:
